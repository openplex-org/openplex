/* Functions to compute SHA1 message digest of files or
   memory blocks according to the NIST specification FIPS-180-1.

   Copyright (C) 2010 Radu Stefan
   ...
   These functions are taken from coreutils-5.2 in order to allow using GPL v2
   */
...
std::string compute_sha(const void *buffer, size_t len)
{
	...
	char a[128];
	int i;
	sha_buffer((const char *) buffer,len,resblock);
	for (i=0; i<20; i++) sprintf(a+i*2,"%02x",resblock[i]);
	return a;
}
