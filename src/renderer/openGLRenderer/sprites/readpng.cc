...
/*
        std::string dumpname = string("dump_" + std::to_string(i) + string(".png"));

        cv::Mat3b img( imgheight, rwidth, (components == 3) ? CV_8UC3 : CV_8UC4);
        auto size = rwidth * imgheight * components;
        memcpy(img.data, pixels + (i * size), size);
        if (components == 4) {
            cv::cvtColor(img, img, CV_RGBA2BGR);
        }
        cv::imwrite(dumpname, img);
*/
        // bind the texture
        ...
        //     printf("%d %d %d %d\n", components, rwidth, imgheight, (i * imgheight * rwidth  * components));
        //glPixelStorei(GL_UNPACK_ROW_LENGTH, rwidth);
        glTexImage2D(GL_TEXTURE_2D, 0, components, rwidth, imgheight, 0, glcolours, GL_UNSIGNED_BYTE,
                     pixels + (i * imgheight * rwidth * components));
    }
