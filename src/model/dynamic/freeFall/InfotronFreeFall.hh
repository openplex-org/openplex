...
struct InfotronFreeFall : public Dynamic {
    GameState &gameState;
    Index src;
    Index dst;
    const int FRAMES = 30;
    ...
    InfotronFreeFall(GameState &gameState, Index src, Index dst) : gameState(gameState), src(src), dst(dst) {
    }
    ...
    void display(const Renderer &renderer) override {
        ...
        x = alpha(src_x, dst_x);
        y = alpha(src_y, dst_y);
        renderer.paint(gameState, x, y, StaticTile::Infotron, 0, 0);
    }
};
