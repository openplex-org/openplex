            ...
            switch (thh) {
                case TH_ANTICLOCK:
                    if (level.ldt[AHEAD].isfreeormurph()) {
                        // move ahead
                        level.ldt[AHEAD].set(base | TH_MOVEIN | thd,
       timing.enemymove); level.ldt[loc].set(TX_MOVEOUT | thd,
       timing.enemymove); } else { level.ldt[loc].set(base | TH_ANTICLOCK |
       ((thd - 1) & 3), timing.enemyrotate);
                    }
                    return;
                case TH_CLOCKWISE:
                    if (level.ldt[AHEAD].isfreeormurph()) {
                        ...
                        level.ldt[AHEAD].set(base | TH_MOVEIN | thd,
       timing.enemymove); level.ldt[loc].set(TX_MOVEOUT | thd,
       timing.enemymove); } else { level.ldt[loc].set(base | TH_CLOCKWISE |
       ((thd + 1) & 3), timing.enemyrotate);
                    }
                    return;
                default: // MOVE, or INVALID
                    if (level.ldt[AHEAD].isfreeormurph() &&
       !level.ldt[LEFT].isfreeormurph()) {
                        // move ahead
                        level.ldt[AHEAD].set(base | TH_MOVEIN | thd,
       timing.enemymove); level.ldt[loc].set(TX_MOVEOUT | thd,
       timing.enemymove); return;
                    }
                    if (level.ldt[RIGHT].isfreeormurph() &&
       !level.ldt[LEFT].isfreeormurph()) {
                        // turn clockwise
                        level.ldt[loc].set(base | TH_CLOCKWISE | ((thd + 1) &
       3), timing.enemyrotate); return;
                    }
                    // default is same as left-turn
                    level.ldt[loc].set(base | TH_ANTICLOCK | ((thd - 1) & 3),
       timing.enemyrotate); return;
            }*/
  }
};
}  // namespace op